    ...
    artworks.map do |a|
        ...
        if FastImage.size(a.picture.url)
          "<figure data-pos='" + a.position.to_s + "' itemprop='associatedMedia' itemscope itemtype='http://schema.org/ImageObject'>" +
            content_tag(:a, cl_image_tag(a.picture, width: width, height: height, crop: :fit, "itemprop": "thumbnail"), href: a.picture.url.gsub(/upload\//, 'upload/c_fit,h_1500,w_1500/'), data: {size: FastImage.size(a.picture.url).join("x")}, itemprop: "contentUrl", onContextMenu: "return false;") +
