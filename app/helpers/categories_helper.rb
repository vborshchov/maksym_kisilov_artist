    ...
    artworks.map do |a|
        ...
        if FastImage.size(a.picture.url)
          "<figure data-pos='" + a.position.to_s + "' itemprop='associatedMedia' itemscope itemtype='http://schema.org/ImageObject'>" + 
            content_tag(:a, cl_image_tag(a.picture, width: 2000, height: 200, crop: :fit, "itemprop": "thumbnail"), href: a.picture.url, data: {size: FastImage.size(a.picture.url).join("x")}, itemprop: "contentUrl") + 
