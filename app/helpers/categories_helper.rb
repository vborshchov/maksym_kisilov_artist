    ...
    artworks.map do |a|
        ...
        if p_size
          "<figure data-pos='" + a.position.to_s + "' itemprop='associatedMedia' itemscope itemtype='http://schema.org/ImageObject'>" +
            content_tag(:a, cl_image_tag(a.picture, width: width, height: height, crop: :fit, quality: 50, "itemprop": "thumbnail", alt: a.name), href: p_url, data: {size: FastImage.size(p_url).join("x")}, itemprop: "contentUrl", onContextMenu: "return false;") +
